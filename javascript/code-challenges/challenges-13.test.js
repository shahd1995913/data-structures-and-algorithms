...
const longestString = (TheArrayOne) => {

const VaribleOne = TheArrayOne.map(y => {
...
const varibleTow = Math.max(...VaribleOne);
...
let Serch = TheArrayOne.find(y => {

  return y.length === varibleTow

})


return TheArrayOne.indexOf(Serch)
...
const firstLetters = (TheArray) => {
  
   let varible = TheArray.map((valueMap) => valueMap.charAt(0));

// The map method will literally 'map' a function call onto each element in the array
...
const findHappiness = (ArrayThree) => {

//You can use .filter() to filter the array down into a new array with strings that have a .length less than 6 characters.

  return  ArrayThree.filter((var3) => var3.includes(":)"));  // filter is a looping mechanism. But yes you can manually achieve this using for. Create dummy array and push necessary element in it and return it
...
Write a function named standardizePhoneNumbers that takes in an array of phone number strings in (XXX) XXX-XXXX format and 
...
let standardizePhoneNumbers = (arrayfour) => {
  const Numbers = '0987654321'
// createPhoneNumber([1,2,3,4,5,6,7,8,9,0]) // returns "(123) 456-7890"
  const ArrayOfTheAllNumB = []
  ...
  for (let a = 0; a < arrayfour.length; a++) {
// Write a function that accepts an array of 10 integers (between 0 and 9), that returns a string of those numbers in the form of a phone number.
      const ArrayOfNUMB = []

      for (let b = 0; b < arrayfour[a].length; b++) {  

          if (Numbers.includes(arrayfour[a][b])) {

            ArrayOfNUMB.push(arrayfour[a][b])    
              
          }            
      }
      ArrayOfTheAllNumB.push(ArrayOfNUMB.join(''))   
      
  }
  return ArrayOfTheAllNumB
};
...
let onlyOddChars = (xx) => {
 // .reduce() to create the two arrays you seem to want.
...
// console.log(encrypt("This is a test!"));
    const ArrayofOddChar = []
   for (let n = 1; n < xx.length; n+=2) {
...
let  allHappy = (TheArrSix) => {
  // Solution code here...

  let  BooleanVal = true; // => false

// letters.includes('a', 1); 
  let ARR = TheArrSix.filter(c => c.includes(':)'));
 // TheArrSix.some(item => shallowEqual(BooleanVal, value));
  if (ARR.length != TheArrSix.length) {
