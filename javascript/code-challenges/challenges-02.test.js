...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach(e => { 
    let NewItem = Math.pow(e, 3);
    raisedToTheThirdArr.push(NewItem)
  });
...
const addOne = (arr) => {
  // Solution code here...
  let AddOne = arr.map((valuees) => valuees +1)
  return AddOne
};
...
iterates over the array and returns a new array. The returned array should contain the result of raising 2 to the power of the original input element.
...
const forEachTwoToThe = (arr) => {
  ...
  let LoopTwo = [];
  arr.forEach(element => { 
    let newElemnt = Math.pow(2, element);
    LoopTwo.push(newElemnt)
  });
  return LoopTwo
...
const mapTwoToThe = (arr) => {
  // Solution code here...
    ...
    let arrayMap = arr.map(element => Math.pow(2,element))
    return arrayMap
