...
Write a function named screenForNames that takes in an array 
of strings and uses Regex to create a new array of only those strings that match the following rules:
...
const screenForNames = (arr) => 
{
  ...
  return arr.filter(Regex =>
    // 1. the name must begin with Mr., Mrs., Ms., Dr. followed by a space
    // 2. the name must contain only letter characters (white spaces are ok)
     /^((Mr|Dr||Mrs|Ms).\s)[A-Z]/.test(Regex));
...

{
  ...
  return arr.map((strings) => 

// returns an array of strings with the first character in upper case and the rest as is.

  strings[0].toUpperCase() + strings.slice(1));
...
let biggerThanLuke = (arr) => {
  ...
  { // names of the characters whose mass is greater than Luke's.

    if (arr[value].mass >= 76) {

      combined.push(arr[value].name)
...
//  "Lando Calrisian - Boba Fett - Princess Amidala".
...
const isSecure = (URL_secure) => {

  return URL_secure.startsWith("https://");
